...
namespace _230503.AStarPractice
{
    internal class Astar
    {
        ...
        // 반환형을 bool로 쓰는 이유는 방문한 경로를 true와 false로 구분짓기 위함
        public static bool PathFinding(bool[,] tileMap, Point start, Point end, out List<Point> path)
        {
            ...
            PriorityQueue<ASNode, int> nextPointPQ = new PriorityQueue<ASNode, int>();        // 우선순위 큐를 사용해 f값이 가장 적은 순서대로 꺼내기 위함
            ...
            while (nextPointPQ.Count > 0)
            {
                ...
                for (int i = 0; i < Direction.Length; i++)
                {
                    ...
                    if (x < 0 || x >= xSize || y < 0 || y >= ySize)
                        continue;                   // 생략(건너 뜀)
                                                    // 탐색할 수 없는 정점일 경우
                    else if (tileMap[y, x] == false)        // 타일 맵에서의 좌표위치가 false인 경우 즉, 갈 수 없는 경우 
                        continue;
                    // 이미 방문한 정점일 경우
                    else if (visited[y, x])
                        continue;
        ...
        // 휴리스틱 (Heuristic) : 최상의 경로를 추징하는 순위값, 휴리스틱에 의해 경로탐색 효율이 결정됨
        ...
        private class ASNode
        {
            ...
            public Point? parent;       // 이 정점을 탐색한 정점 parent의 좌표가 없을수도 있으니 ? Nullable 설정
